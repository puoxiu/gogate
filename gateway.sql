...
-- --------------------------------------------------------
-- 数据库: `go_gateway`
CREATE DATABASE IF NOT EXISTS `go_gateway` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;
USE `go_gateway`;
...
--
-- 表的结构 `gateway_app` 存储接入网关的租户（应用）配置,用于租户身份认证、请求限流和 IP 白名单控制
--
...
--
-- 表的结构 `gateway_service_access_control` 为单个网关服务配置权限控制规则,包括权限开关、IP 黑白名单、限流策略
--
