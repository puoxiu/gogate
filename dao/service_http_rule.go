...
func (t *HttpRule) Find(c *gin.Context, tx *gorm.DB, search *HttpRule) (*HttpRule, error) {
	model := &HttpRule{}
	// err := tx.WithContext(c.Request.Context()).Where(search).Find(model).Error
	err := tx.WithContext(c.Request.Context()).Where(search).First(model).Error
	return model, err
}

func (t *HttpRule) Save(c *gin.Context, tx *gorm.DB) error {
	if err := tx.WithContext(c.Request.Context()).Save(t).Error; err != nil {
		return err
	}
	return nil
}
